...
void parsePacket2(const char* str, struct Packet *packet) {

    unsigned int member = 1;
    char *total_frag;   //member 1
    char *frag_no;      //member 2
    char *size;         //member 3
    char *filename;     //member 4

    int i1 = 0; //first index of a member
    int i2 = 0; //index of colon after the member 

    for (; (i2 < 2000) && (member < 5); i2++)
    {
        if (str[i2] == ':')
        {
            if (member == 1)    //total_frag
            {
                total_frag = malloc(i2 - i1);
                memcpy(total_frag, str + i1, i2 - i1);
            }
            else if (member == 2)    //frag_no
            {
                frag_no = malloc(i2 - i1);
                memcpy(frag_no, str + i1, i2 - i1);
            }
            else if (member == 3)    //size
            {
                size = malloc(i2 - i1);
                memcpy(size, str + i1, i2 - i1);
            }
            else if (member == 4)    //filename
            {
                filename = malloc(i2 - i1);
                memcpy(filename, str + i1, i2 - i1);
            }
            else{printf("ERROR IN PP2.\n"); exit(1);}
...
int main(int argc, char *argv[]){
    ...
    if (recvfrom(sock_fd, buffer, 2000, 0, (struct sockaddr *) &client_addr, &length) == -1){
    ...
    while (pack_num < packs_to_rec){
        ...
        //ONE WAY TI DO IT
        parsePacket2(buffer, &new_packet);

        //parse_packet(buffer, &new_packet);
        if (new_packet.frag_no == 1){
            outStream = fopen("servercopy1.jpg", "wb");
        }
        ...
        if (currentPacket == 1){
            outStream = fopen("servercopy1", "wb");
        }
